...
featured_image: ../assets/images/post/3.png
draft: false
---
...
[over](https://pan.baidu.com/s/1H9-yzXXKt4gIXcQgE6u8jA?pwd=geek)
提取码:geek
...
retn
```
第一个系统调用的`系统调用号`可以被我们的一字节溢出`覆写`，所以我们可以控制这个系统调用。程序原本的流程是:`open(filename,0,0)`,filename是用户输入的字符串，刚好8字节给我们输入`"/bin/sh\x00"`，于是我们完全可以将这个调用控制为：`execv("/bin/sh\x00",0,0)`,攻击成功
