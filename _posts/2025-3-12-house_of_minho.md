...
![1](https://raw.githubusercontent.com/4ak5ra/image/main/常规.png)

利用 `orange` 的思路，把 top 改小再想办法申请一个大堆块把 top 放到 unsortedbin 里面(topchunk 是页对其的，改小后重新分配的 topchunk 会在下一页，中间会有一大段断掉的空间)。利用 scanf 等输入型 io 函数的缓冲机制来 malloc 这个大 heap。泄漏就直接用 scanf 的%s 机制配合题目本身的漏洞就行了(覆盖掉要泄漏数据之前的 0)  
```python
...
count 一直为 1 肯定做不到任意写，所以需要利用 malloc 的整理机制把 smallbin 转移到 tcache 中来增加 count 的值。然后我们想办法利用 unsorted bin 的指针来伪造这样 small bin size 的 unsorted bin。我们只能通过 1 去覆写 2, 所以应该在 2 的 bin 中伪造两个 bin，所以这个 small bin size 就是 0x90。再利用 scanf 触发 malloc    
...
利用 consolidate backward 又需要经历一次把 unsorted bin 取出来的操作，但是此时我们可以利用前面 orange 时 ptmalloc 已经构造出的保护堆块。之需要在这两个堆块的下面再加一个标记这 inuse 的空间即可，此时再利用 scanf 的缓冲机制即可：需要了解 scanf 的源码，为什么 scanf 申请出的缓冲区(0x1000)没有满却写到了 topchunk 里面去了.


![2](https://raw.githubusercontent.com/4ak5ra/image/main/scanf.png)
...
free()
```
![3](https://raw.githubusercontent.com/4ak5ra/image/main/bin.png)    
...
def get_addr(arch):
    if arch == 64:
        ...
        #return u64(io.recv()[-8:].ljust(8, b'\x00')) #所有地址只用到 6 字节
    else:
